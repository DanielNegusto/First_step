...
from src.reports import save_to_file, spending_by_category, category_spending


def test_save_to_file():
    # Создаем тестовую функцию-отчет
    @save_to_file()
    def test_func():
        return 'Test result'
    # Вызываем функцию-отчет
    test_func()
    # Проверяем, что результат был сохранен в файл
    with open('reports.log', 'r') as file:
        assert file.read() == 'Test result'
...
def transactions():
    data = {
        ...
        'Дата операции': [datetime(2022, 1, 1), datetime(2022, 1, 15), datetime(2022, 2, 1), datetime(2022, 3, 1),
                          datetime(2022, 4, 1)],
...
def test_spending_by_category_with_no_transactions_in_last_90_days():
    ...
    transactions = pd.DataFrame({
        'Категория': ['Еда'],
        'Дата операции': [datetime(2022, 1, 1)],
        'Сумма операции': [-100]
    })
...
def df():
    data = {
        'Категория': ['Еда', 'Транспорт', 'Еда', 'Товары', None],
        'Расходы': [100, 200, 50, 150, 0]
    }
    return pd.DataFrame(data)
